...
script_authors("Nishikinov", "Ningaro")
    ...
    for _, handle in ipairs(getAllVehicles()) do
    	if handle ~= mycar and doesVehicleExist(handle) and isCarOnScreen(handle) then
              ...
              _, id = sampGetVehicleIdByCarHandle(handle)
							...
							color = color:sub(1, -3)
      				renderFontDrawText(font, vehName .. "[".. id .."]" .. " {" .. color .. "}Цвет{FFFFFF}(" .. primaryColor .. ")", X - 10, Y - 20, 0xFFFFFFFF)
      			end
      	end
  	end
...
function main()
	...
	while not isSampAvailable() do wait(100) end
	   sampAddChatMessage("[Veh info loaded]", - 1)
	   while true do
	   	wait(0)
